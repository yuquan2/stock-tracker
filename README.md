...
每个 `D2` 交易日会生成两类中文表头 CSV：

- `data/YYYYMMDD.csv`：完整原始数据日志，包含每只非 ST、非科创板、非北交所股票的 D0/D1/D2 日期、开盘价、收盘价、最高价、最低价及成交量。仅保留三天日线均完整的股票。
- `results/YYYYMMDD.csv`：在同一份原始数据日志上应用全部形态规则后的匹配结果。

同日期的 `data/YYYYMMDD.csv` 已存在时，脚本会直接复用本地数据并重新筛选，不访问网络；因此调整筛选条件后可快速重跑。需要强制更新原始数据时使用 `python -m a_share_screener.runner --refresh-data`。数据通过 AkShare 的交易日历、腾讯 A 股现货列表与腾讯不复权历史日线接口获取，无需环境变量或密钥。
