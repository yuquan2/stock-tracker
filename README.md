...
价格匹配必须完全相同：例如 D1 收盘为 `10.50` 时，D2 最高价必须为 `10.50`。
...
- `results-exact/YYYY/MM/YYYYMMDD.csv`：以最近三个交易日快照按严格价格匹配筛选出的结果，按形态日期的年份与月份分组。各字段以 `D0(0831)`、`D1(0901)`、`D2(0902)` 形式标注实际交易日期。
...
工作流位于 [`.github/workflows/screen.yml`](.github/workflows/screen.yml)，会在每个工作日北京时间 17:20 运行，也可在 **Actions** 页面手动触发。无需配置 GitHub Secret。它会先由脚本核验实际交易日，随后只提交 `results-exact/` 下有变化的 CSV。
